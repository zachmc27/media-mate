...
router.get("/details/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (type !== "movie" && type !== "tv") {
      return res
        .status(400)
        .json({ error: "Invalid type. Type must be 'movie' or 'tv'." });
    }
...
router.get("/discover/:type", async (req: Request, res: Response) => {
  ...
  if (type !== "movie" && type !== "tv") {
    return res
      .status(400)
      .json({ error: "Invalid type, use either 'movie' or 'tv'." });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "An error occurred retrieving media for the Discovery" });
  }
});
...
router.get("/movie/recent", async (_req: Request, res: Response) => {
    ...
    try {
        ...
        const recentMoviesResponse = await fetch( `${baseUrl}/movie/now_playing`, options )

        if (!recentMoviesResponse.ok) {
            throw new Error("Failed to fetch data from TMDB for Recently Released");
        }

        const moviesData = (await recentMoviesResponse.json()) as TMDBResponse;

        const mediaResults = moviesData.results.map((item: any) => {
            return new Media({
                id: item.id,
                title: item.title || item.name,
